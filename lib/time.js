...
var Time = function() {
		...
		if(minute!=_this.last_minute) {
			...
			if(config.military_time===false) {
				if(_this.hour12<10)
					...
					_this.display.writeDigit(0, Math.floor(_this.hour12/10));
			}
	...
	this.brightness = function(val) {
		function set_brightness() {
			_this.display.setBrightness(val);
		}

		// Make sure the brightness gets set
		// (it doesn't seem to always work on the first try)
		if(!!val) {
			setTimeout(set_brightness, 1);
			setTimeout(set_brightness, 100);
			setTimeout(set_brightness, 250);
		}
	}
	...
	this.sensor_brightness = function() {
		...
		_this.sensor.getLight0(function(err, data) {
			...
			var brightness1 = _map(data, config.light_min, config.light_max, 1, 16);

			if(brightness1 != _this.last_brightness1) {
				if(config.verbose===true)
					console.log('BRIGHTNESS', brightness0, '=>', brightness1);

				_this.brightness(brightness1);
				_this.emit("light", brightness0);
				_this.last_brightness1 = brightness1;
			}
		});
